  ...
  public void run()
  {
    ...
    else
    {
      ...
      try
      {
        ...
        while (rs.next()) {
          if ((Constants.InterestType.equalsIgnoreCase("players")) || (!Constants.Banking)) {
            players.add(rs.getString("username")); continue;
          }
    ...
    try
    {
      ...
      if ((Constants.InterestType.equalsIgnoreCase("players")) || (!Constants.Banking)) {
        ...
        for (String name : bankPlayers.keySet()) {
          ...
          if (account != null) {
            Holdings holdings = account.getBankHoldings(((Integer)bankPlayers.get(name)).intValue());
            ...
            if (holdings != null) {
              ...
              ps.setDouble(1, balance + amount);
              ps.setString(2, name);
              ps.setInt(3, ((Integer)bankPlayers.get(name)).intValue());
              ps.addBatch();
