...
public class Constants {
	...
	public static void load(File file) {
		...
		try {
			...
			Holdings = config.getDouble("System.Default.Account.Holdings",
					Holdings);

			Banking = config.getBoolean("System.Banking.Enabled", Banking);
			BankingMultiple = config.getBoolean(
					"System.Banking.Accounts.Multiple", BankingMultiple);
			BankName = config.getString("System.Default.Bank.Name", BankName);
			BankMajor = config
					.getStringList("System.Default.Bank.Currency.Major");
			BankMinor = config
					.getStringList("System.Default.Bank.Currency.Minor");
			BankHoldings = config.getDouble(
					"System.Default.Bank.Account.Holdings", BankHoldings);
			BankFee = config.getDouble("System.Default.Bank.Account.Fee",
					BankFee);

			Logging = config.getBoolean("System.Logging.Enabled", Logging);

			FormatMinor = config.getBoolean("System.Formatting.Minor",
					FormatMinor);
			FormatSeperated = config.getBoolean("System.Formatting.Seperate",
					FormatSeperated);

			Interest = config.getBoolean("System.Interest.Enabled", Interest);
			InterestOnline = config.getBoolean("System.Interest.Online",
					InterestOnline);
			InterestType = config.getString("System.Interest.Amount.On",
					InterestType);
			InterestAnn = config.getBoolean("System.Interest.Announce.Enabled",
					InterestAnn);
			InterestSeconds = config.getInt("System.Interest.Interval.Seconds",
					InterestSeconds);
			InterestPercentage = config.getDouble(
					"System.Interest.Amount.Percent", InterestPercentage);
			InterestCutoff = config.getDouble("System.Interest.Amount.Cutoff",
					InterestCutoff);
			InterestMin = config.getDouble("System.Interest.Amount.Minimum",
					InterestMin);
			InterestMax = config.getDouble("System.Interest.Amount.Maximum",
					InterestMax);

			DatabaseType = config.getString("System.Database.Type",
					DatabaseType);

			SQLDatabase = config.getString("System.Database.Settings.Name",
					SQLDatabase);
			SQLTable = config.getString("System.Database.Settings.Table",
					SQLTable);

			SQLHostname = config.getString(
					"System.Database.Settings.MySQL.Hostname", SQLHostname);
			SQLPort = config.getString("System.Database.Settings.MySQL.Port",
					SQLPort);
			SQLUsername = config.getString(
